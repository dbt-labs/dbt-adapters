...
# Bucket partition present: auto target filter should be disabled
models__batch_merge_bucket_guard_sql = """
{{ config(
    materialized='incremental',
    table_type='iceberg',
    incremental_strategy='merge',
    unique_key='id',
    partitioned_by=['DAY(date_column)', 'bucket(id, 5)'],
    force_batch=True
) }}
...
class TestBatchMergeBucketGuard:
    """When bucket partitions are present, auto target partition filter should be disabled."""

    @pytest.fixture(scope="class")
    def models(self):
        return {
            "test_bucket_guard.sql": models__batch_merge_bucket_guard_sql,
        }

    def test__bucket_guard_disables_auto_target_filter(self, project, capsys):
        relation_name = "test_bucket_guard"
        count_query = f"select count(*) as records from {project.test_schema}.{relation_name}"
        ...
        # Auto target filter should NOT be present (bucket guard)
        assert "date_trunc('day', target.date_column)" not in out, (
            "Auto target partition filter should be disabled when bucket partitions are present"
        )
