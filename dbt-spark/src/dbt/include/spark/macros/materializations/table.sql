...
{% if submission_method == 'spark_master' %}
import sys
location = sys.argv[1]
...
coalesce_output = {{ config.get('coalesce_output', False) }}
write_options = {{ config.get('write_options', {}) }}

if file_format in ('delta', 'hudi', 'iceberg'):
    print(f"Writing {file_format} table '{{ target_relation }}' to path {table_location}")
    if coalesce_output:
        print("coalesce_output is ignored when using Iceberg format!")
    writer = df.write.mode("overwrite").format(file_format).option("overwriteSchema", "true").option("path", table_location)
    {% for key, value in config.get('write_options', {}).items() %}
    writer = writer.option("{{ key }}", "{{ value }}")
    {% endfor %}
    writer.saveAsTable("{{ target_relation }}")
else:
    print(f"Writing dataframe to path {table_location} as {file_format}")
    df = df.coalesce(1) if coalesce_output else df
