...
class TestSnowflakeUnitTestingTypesCaseInsensitive(BaseUnitTestingTypes):
    @pytest.fixture
    def data_types(self):
        # sql_value, yaml_value
        return [
    ...
    def dbt_profile_target(self, dbt_profile_target_case_insensitive):
        return dbt_profile_target_case_insensitive


class TestSnowflakeUnitTestCaseInsensitivityCaseInsensitive(BaseUnitTestCaseInsensivity):
...
class TestSnowflakeUnitTestInvalidInputCaseInsensitive(BaseUnitTestInvalidInput):
    @pytest.fixture(scope="class", autouse=True)
    def dbt_profile_target(self, dbt_profile_target_case_insensitive):
        return dbt_profile_target_case_insensitive
