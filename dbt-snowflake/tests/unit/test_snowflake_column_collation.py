...
class TestSnowflakeColumnCollationCanExpandTo(unittest.TestCase):
    """Test cases for can_expand_to method in SnowflakeColumn."""

    STRING_TYPES = ("varchar", "character varying", "character", "varchar")

    def test_can_expand_to_size_increase_across_all_string_types(self):
        """A size increase should be considered expandable for all Snowflake string synonyms."""
        for type_name in self.STRING_TYPES:
            with self.subTest(type_name=type_name, message=f"test size increase {type_name}"):
                source_column = SnowflakeColumn.from_description("test_col", f"{type_name}(10)")
                target_column = SnowflakeColumn.from_description("test_col", f"{type_name}(20)")
                assert source_column.can_expand_to(target_column) is True

    def test_can_expand_to_collation_mismatch_source_none_target_present_across_all_string_types(
        self,
    ):
        """
        If the target has collation but the source does not, we should not treat it as "expandable"
        (i.e. do not trigger an ALTER COLUMN SET DATA TYPE that would drop collation).
        """
        for type_name in self.STRING_TYPES:
            with self.subTest(type_name=type_name, message=f"test collation mismatch {type_name}"):
                source_column = SnowflakeColumn.from_description("test_col", f"{type_name}(10)")
                target_column = SnowflakeColumn.from_description(
                    "test_col", f"{type_name}(10) COLLATE 'en-ci'"
                )
                assert source_column.can_expand_to(target_column) is False

    def test_can_expand_when_collation_is_the_same_across_all_string_types(self):
        """equal collation should count as expandable."""
        for type_name in self.STRING_TYPES:
            with self.subTest(type_name=type_name, message=f"test collation match {type_name}"):
                source_column = SnowflakeColumn.from_description(
                    "test_col", f"{type_name}(10) COLLATE 'en-ci'"
                )
                target_column = SnowflakeColumn.from_description(
                    "test_col", f"{type_name}(20) COLLATE 'en-ci'"
                )
                assert source_column.can_expand_to(target_column) is True
