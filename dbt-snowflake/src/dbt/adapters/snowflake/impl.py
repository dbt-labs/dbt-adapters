...
class SnowflakeAdapter(SQLAdapter):
    ...
    def __init__(self, config, mp_context) -> None:
        ...
        self.add_catalog_integration(constants.DEFAULT_LOCAL_CATALOG)
    ...
    def _parse_list_relations_result(self, result: "agate.Row") -> SnowflakeRelation:
        ...
        table_format = (
            ...
            else constants.LOCAL_TABLE_FORMAT
        )
    ...
    def _get_adapter_specific_run_info(cls, config: RelationConfig) -> Dict[str, Any]:
        ...
        if config and hasattr(config, "_extra"):
            ...
            if _table_format := config._extra.get("table_format"):  # type:ignore
                ...
                # no table_format and no catalog definitely means local table
                run_info["table_format"] = constants.LOCAL_TABLE_FORMAT
            elif catalog == constants.DEFAULT_LOCAL_CATALOG.name:  # type:ignore
                # if the user happens to set the catalog to the local catalog, catch that
                run_info["table_format"] = constants.LOCAL_TABLE_FORMAT
            else:  # catalog is set, and it's not the local catalog
