...
class TestQueryGroup(TestCase):
    ...
    def test_open_executes_set_query_group_when_configured(self):
        ...
        mock_handle.cursor.return_value = mock_cursor_cm
    ...
    @mock.patch("redshift_connector.connect", MagicMock())
    def test_open_does_not_execute_set_query_group_when_not_configured(self):
        mock_cursor = MagicMock()
        ...
        set_query_calls = [
            ...
            if c.args and "SET query_group" in str(c.args[0])
        ]
        assert len(set_query_calls) == 0
