...
class TestMergeRelationTables:
    ...
    def test_table_type_not_overridden_by_mv(self, adapter):
        """A 'table' should not be changed even if its key matches an MV."""
        all_relations = _make_table(
            [
                ("db", "my_table", "public", "table"),
            ]
        )
        materialized_views = _make_table(
            [
                ("db", "my_table", "public", "materialized_view"),
            ]
        )
