...
{% macro redshift__list_relations_without_caching(schema_relation) %}
  {% if redshift__use_show_apis() %}
    ...
    {% call statement('_dbt_all_relations', fetch_result=True) -%}
      select
    ...
    {% call statement('_dbt_materialized_views', fetch_result=True) -%}
      select
    ...
    {% set all_relations = load_result('_dbt_all_relations').table %}
    {% set materialized_views = load_result('_dbt_materialized_views').table %}
